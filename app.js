...
let visibilidadCapas = { luminarias: true, arbolado: true, vialidades: true, reclamos: true, cordon: true, banquina_vereda: true, cuneta: true };
...
if (!map.getSource('arbolado-source')) {
    ...
    map.addLayer({
        ...
        paint: {
            ...
            // 2. Transparencia en el color de relleno
            'circle-opacity': 0.85, 
            
            // 3. Línea de borde
            'circle-stroke-width': 0.5, 
            
            // 4. color de Borde  semi-transparente (suavizar contraste)
            'circle-stroke-color': 'rgba(15, 23, 42, 0.4)', 
            
            'circle-color': [
                'match',
                ['upcase', ['coalesce', ['get', 'ESTADO_S'], ['get', 'estado_s'], ['get', 'ESTADO'], 'OTROS']],
        ...
        layout: { 'visibility': visibilidadCapas.arbolado ? 'visible' : 'none' }
    });
}
...
capasMapeo.forEach(layerId => {
    ...
    map.on('click', layerId, (e) => {
        ...
        // 1. CAPA ARBOLADO
        // -------------------------------------------------------------
        if (capaOrigen === 'arbolado-layer') {
            if (labelIdEl) labelIdEl.innerText = 'ID Árbol';
            if (labelElemEl) labelElemEl.innerText = 'Activo';

            document.getElementById('info-id').innerText = props.id || props.ID || 'N/A';
            document.getElementById('info-elemento').innerText = 'Arbolado';

            setCampo('info-tipo', 'label-tipo', props.especie || props.ESPECIE, 'Especie');
            setCampo('info-potencia', 'label-potencia', props.dimension || props.DIMENSION, 'Dimensión');
            setCampo('info-marca', 'label-marca', props.estado_s || props.ESTADO_S || props.ESTADO, 'Estado Vegetativo');
            setCampo('info-modelo', 'label-modelo', props.tronco || props.TRONCO, 'Tronco');
            setCampo('info-calle', 'label-calle', props.calle || props.CALLE, 'Calle');
            setCampo('info-soporte', 'label-soporte', props.base || props.BASE, 'Base');
            
            // Campos secundarios (Expandibles) 
            setCampo('info-funcion', 'label-funcion', null, '-');
            setCampo('info-brazo', 'label-brazo', null, '-');
            setCampo('info-streetview', 'label-streetview', props.f_street_v || props.F_STREET_V, 'F. Street View');
...
function configurarBotonesPrenderApagar() {
    ...
    configurarBotonIndividual(
        'btn-mod-arbolado', 'drop-arbolado', 'arbolado-layer', 'arbolado', 
    ...
    if (btnTodos) {
        ...
        const llavesCapas = ['luminarias', 'arbolado', 'vialidades', 'cordon', 'banquina_vereda', 'cuneta', 'reclamos'];
        const idsBotones = {
            luminarias: 'btn-mod-luminarias',
            arbolado: 'btn-mod-arbolado',
...
// ===== ARBOLADO =====
    const arboles = capasData.arbolado.features || [];
    //const arboles = window.datosActualesParaKPI.arbolado.features || [];
    const totalArb = arboles.length;
    ...
    document.getElementById('kpi-arb-total').innerText = totalArb.toLocaleString();

    document.getElementById('kpi-arb-bueno').innerText = 
        `${bueno.toLocaleString()} (${totalArb ? ((bueno / totalArb) * 100).toFixed(1) : 0}%)`;

    document.getElementById('kpi-arb-regular').innerText = 
        `${regular.toLocaleString()} (${totalArb ? ((regular / totalArb) * 100).toFixed(1) : 0}%)`;

    document.getElementById('kpi-arb-malo').innerText = 
        `${malo.toLocaleString()} (${totalArb ? ((malo / totalArb) * 100).toFixed(1) : 0}%)`;

    document.getElementById('kpi-arb-otros').innerText = 
        `${otros.toLocaleString()} (${totalArb ? ((otros / totalArb) * 100).toFixed(1) : 0}%)`;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderizarEstadisticas() {
        ...
        const totalArb = capasData.arbolado?.features?.length || 0;
        const totalRec = capasData.reclamos?.features?.length || 0;
        const totalCord = capasData.cordon?.features?.length || 0;

        // Conteo por ESPECIE
        const especies = {};
        (capasData.arbolado?.features || []).forEach(f => {
            const esp = String(f.properties?.ESPECIE || f.properties?.especie || f.properties?.Especie || 'Sin especie').trim();
            const clave = esp === '' || esp === 'NULL' || esp === 'null' ? 'Sin especie' : esp;
            especies[clave] = (especies[clave] || 0) + 1;
        });
        
        // Top 8 especies ordenadas
        const topEspecies = Object.entries(especies)
            .sort((a, b) => b[1] - a[1])
            .slice(0, 8);
        
        const maxEsp = topEspecies.length ? topEspecies[0][1] : 1;
        ...
        const maxVal = Math.max(totalLum, totalArb, totalRec, totalCord, totalKm) || 1;
        ...
        // --- LATERAL VIAL (km) ---
        let kmCordonStats = 0, kmBanquinaStats = 0, kmCunetaStats = 0;
        (capasData.cordon?.features || []).forEach(f => {
            const p = f.properties || {};
            if (String(p.cordon || p.CORDON || '').trim().toUpperCase() === 'SI') {
                kmCordonStats += parseFloat(p.km_cordon || p.KM_CORDON || 0);
            }
        });
        (capasData.banquina_vereda?.features || []).forEach(f => {
            const p = f.properties || {};
            if (String(p.banq_vrda || p.BANQ_VRDA || '').trim().toUpperCase() === 'SI') {
                kmBanquinaStats += parseFloat(p.km_b_v || p.KM_B_V || 0);
            }
        });
        (capasData.cuneta?.features || []).forEach(f => {
            const p = f.properties || {};
            if (String(p.cuneta || p.CUNETA || '').trim().toUpperCase() === 'SI') {
                kmCunetaStats += parseFloat(p.km_cuneta || p.KM_CUNETA || 0);
            }
        });
        const totalKmStats = kmCordonStats + kmBanquinaStats + kmCunetaStats;
        ...
        statsContent.innerHTML = `
            ${seccionCard('Resumen General', `
                ...
                ${barra('Lateral Vial (km)', parseFloat(totalKmStats.toFixed(1)), 'bg-slate-500')}
            ...
            ${seccionCard('Lateral Vial (km)', `
                <div class="space-y-0.5">
                    ${filaLimpia('Cordón', kmCordonStats.toFixed(1) + ' km', 'bg-slate-600')}
                    ${filaLimpia('Banquina/Vereda', kmBanquinaStats.toFixed(1) + ' km', 'bg-slate-500')}
                    ${filaLimpia('Cuneta', kmCunetaStats.toFixed(1) + ' km', 'bg-slate-400')}
                </div>
            `)}

            ${seccionCard('Arbolado por Especie (Top)', `
                <div class="space-y-3 pt-1">
                    ${topEspecies.map(([nombre, cant]) => {
                        const pct = (cant / maxEsp) * 100;
                        return `
                            <div class="space-y-1">
                                <div class="flex justify-between items-center text-xs">
                                    <span class="font-medium text-slate-700 dark:text-slate-300 truncate max-w-[150px]" title="${nombre}">${nombre}</span>
                                    <span class="font-mono font-semibold text-slate-900 dark:text-slate-100">${cant.toLocaleString()}</span>
                                </div>
                                <div class="w-full bg-slate-100 dark:bg-slate-800 rounded-md h-1.5 overflow-hidden">
                                    <div class="bg-emerald-600 h-1.5 rounded-md transition-all duration-500 ease-out" style="width: ${pct}%"></div>
                                </div>
                            </div>
                        `;
                    }).join('')}
